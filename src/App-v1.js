...
function MovieDetails({ selectedID, handleCloseMovie, onAddWatched, watched }) {
  ...
  function onAddToWatched(movie) {
    const WatchedMovieDetails = {
      imdbID: selectedID,
      Title: movie.Title,
      Year: movie.Year,
      Poster: movie.Poster,
    ...
    onAddWatched(WatchedMovieDetails);
    handleCloseMovie();
  }
  ...
  useEffect(function () {
    async function fetchMovieDetails() {
      try {
        setError("");
        setIsLoading(true);
        const res = await fetch(`http://www.omdbapi.com/?apikey=44386d09&i=${selectedID}`);
  ...
  const { Title: title
    , Year: year, imdbRating: rating, Director: director
    , Poster: poster,
    Genre: genre,
    Plot: plot,
    Runtime: runtime
        ...
        <>
          ...
          <section>
            <div className="rating">
              {isWatched ? (
                <p>You rated this movie {isWatched.userRating} ⭐️</p>
              ) : (<>
                <StarRating
                  maxRating={10}
                  size={24}
                ...
                />
                <button className="btn-add" onClick={() => onAddToWatched(movie)}>+ Add to list</button>
              </>)}
